...
boolean scl_read()
{
	return ((I2C_mReadSlaveReg0(i2c_base_address,0)&(1<<(28)))&&1 );
}

boolean sda_read()
{

	return ((I2C_mReadSlaveReg0(i2c_base_address,0)&(1<<(29)))&&1);
}
void sda_write(boolean val)
{
	...
	if(val)
		...
		((scl_last<<30)&(~(1<<31)));
	sda_last=val;
	I2C_mWriteSlaveReg0(i2c_base_address, 0, out);
	delay_ns(10);
 ...
 char  i2c_master_write_simple(u8 slave_address,u8 slave_register,u8 slave_data)
{
 ...
 int  i2c_master_read_simple_2Byte(u8 slave_address,u8 slave_register){
	u16 data;
	i2c_start();
	i2c_write(slave_address);
	i2c_write(slave_register);
